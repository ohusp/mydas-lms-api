...
class PharmAccount extends Component {
  constructor(props) {
    super(props);
    // ////////////////MODAL    
  ...
  componentDidMount()
  {
    axios.get(`/api/pharm/order/account/`+this.state.id+`?token=${this.state.token}`)
    .then(response => {
      return response;
    })
    .then(json => {
  ...
  handlePageChange(pageNumber) {
    ...
    axios.get(`/api/pharm/order/account/`+this.state.id+`?token=${this.state.token}&page=`+pageNumber)
    .then(response => {
      return response;
    })
    .then(json => {
                    ...
                    <tbody>
                      {
                        ...
                        this.state.applications_list.map(application=>{
                          if(application.status == 1){
                            this.state.status = <Badge color="danger">Product Not Delivered</Badge>;
                          }else if(application.status == 2){
                            this.state.status = <Badge color="primary">Product Delivered</Badge>;
                          }else if(application.status == 3){
                            this.state.status = <Badge color="success">CamMedics Paid</Badge>;
                          }
                          ...
                          return(
                            <tr key={application.id}>
                              ...
                              <td>{name}</td>
                              <td>{application.billing_cart_total}</td>
