...
class Ports extends Component {
  ...
  componentDidMount()
  {
    axios.get(`/api/admin/list_ports?token=${this.state.token}`)
    .then(response => {
  ...
  handlePageChangePorts(pageNumber) {
    ...
    axios.get(`/api/admin/list_ports?token=${this.state.token}&page=`+pageNumber)
    .then(response => {
      return response;
    })
    .then(json => {
  ...
  viewPort(port_id, name) {
    axios.get(`/api/admin/port/get/`+port_id+`?token=${this.state.token}`)
    .then(response => {
    ...
    .then(json => {
      if (json.data.success) {
        ...
        }, this.togglePortView(port_id, name));
      } else {
        this.setState({
  ...
  toggleAccountView = (port_id, name) => {
    // togglePatientRec(id, name) {
  ...
  viewAccount(port_id, name) {
    this.setState({
      port_id: port_id,
      port_name: name,
    });
    axios.get(`/api/admin/port/account/`+port_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("ROI Cartoon");
      console.log(response);
      return response;
    ...
    .then(json => {
      if (json.data.success) {
        ...
        }, this.toggleAccountView(port_id, name));
      } else {
  ...
  handlePageChangeAccount(pageNumber) {
    console.log(`active page is ${pageNumber}`);
    // this.setState({activePage: pageNumber});
    axios.get(`/api/admin/port/account/`+this.state.port_id+`?token=${this.state.token}&page=`+pageNumber)
    .then(response => {
      return response;
    })
    .then(json => {
  ...
  getMessages2(){
    // alert("Paulo");
    axios.get(`/api/port/patient/chat/message/get/`+this.state.patient_id+'/'+this.state.port_id+`?token=${this.state.token}`)
      .then(response => {
        console.log("ROI Cartoon");
        console.log(response);
        return response;
  ...
  sendMessage(){
    ...
    // console.log(testmessage);
      ...
      axios.post(`/api/port/patient/chat/message/send/`+this.state.patient_id+'/'+this.state.port_id+`?token=${this.state.token}`, send_message)
      .then(response => {
        console.log("ROI Cartoon");
        console.log(response);
        return response;
  ...
  uploadSendFile(){
    const url = '/api/port/patient/chat/file/send/'+this.state.patient_id+'/'+this.state.port_id+`?token=${this.state.token}`;
  ...
  getPateintMedicalRec(patient_id, name) {
    ...
    axios.get(`/api/port/patientMedRec/get/`+patient_id+'/'+this.state.port_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("It came back");
      console.log(response);
      return response;
  ...
  viewAppointment(patient_id, name) {
    ...
    // get patient medical records to display in modal
    axios.get(`/api/port/appointment/get/`+patient_id+'/'+this.state.port_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("It came back");
      console.log(response);
      return response;
  ...
  endAppointment() {
    // get patient medical records to display in modal
    axios.get(`/api/port/appointment/end/`+this.state.patient_id+'/'+this.state.port_id+'/'+this.state.appointment_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("It came back");
      console.log(response);
      return response;
  ...
  {   
      ...
      axios.post(`/api/port/patientMedRec/update/`+this.state.patient_id+'/'+this.state.port_id+`?token=${this.state.token}`, med_rec_data)
  ...
  changePortStatus(port_id){
    this.setState({ 
      port_id: port_id
    });
    axios.get(`/api/admin/port/change_status/`+port_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("ROI Cartoon");
      console.log(response);
      return response;
  ...
  changeAccountStatus(account_id){
    ...
    //   port_id: port_id
    // });
    axios.get(`/api/admin/port/change_account_status/`+account_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("ROI Cartoon");
      console.log(response);
      return response;
  ...
  render() {
    ...
    if(this.state.status_port == "0"){ this.state.status_port = "Not activated" }
    if(this.state.status_port == "1"){ this.state.status_port = "Activated" }
      ...
      <div className="animated fadeIn"> 
                    ...
                    <tbody>
                      {
                        ...
                        this.state.ports_list.map(port=>{
                          if(port.status == 1){
                            this.state.port_status = <Button size="sm" onClick={() => this.changePortStatus(port.id)} active color="danger" title="deactivate account" aria-pressed="true"><i className="fa fa-close"></i></Button>;
                          }else{
                            this.state.port_status = <Button size="sm" onClick={() => this.changePortStatus(port.id)} active color="success" title="activate account" aria-pressed="true"><i className="fa fa-check"></i></Button>;
                          }
                          const port_id  = port.id;
                          const name     = port.first_name +" "+port.last_name+" "+port.middle_name;
                          return(
                            <tr key={port.id}>
                              ...
                              <td>{port.email}</td>
                              {/* <td>{port.nationality}</td> */}
                              <td>{port.country}</td>
                              <td>{port.district_province_state}</td>
                              {/* <td>{this.state.status}</td> */}
                              <td>
                                <Button size="sm" onClick={() => this.viewPort(port_id, name)} className="btn-facebook btn-brand icon mr-1 mb-1" title="View port"><i className="fa fa-user"></i></Button>
                                <Button size="sm" onClick={() => this.viewAccount(port_id, name)} className="btn-vine btn-brand icon mr-1 mb-1" title="View port's account"><i className="fa fa-money"></i></Button>
                                <Button size="sm" onClick={() => this.viewPortPatient(port.id)} className="btn-flickr btn-brand icon mr-1 mb-1" title="View port's patient"><i className="fa fa-stethoscope"></i></Button>
                              </td>
                              <td>{this.state.port_status}</td>
                            </tr>
                          )
                        })
                      }
        ...
        <Modal isOpen={this.state.accountView} className={'modal-primary ' + this.props.className} style={{maxWidth: "1000px"}}>
          ...
          <ModalBody>
            ...
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i>{this.state.patient_name} Port's Account
              </CardHeader>
              <CardBody>
                <Row>
                  <Table responsive bordered>
                        ...
                        this.state.account_list.map(account=>{
                          ...
                          return(
                            <tr key={account.id}>
                              ...
                              <td>{name}</td>
                              <td>{account.billing_port_fee}</td>
