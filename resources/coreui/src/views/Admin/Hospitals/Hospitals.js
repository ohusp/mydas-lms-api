...
class Hospitals extends Component {
  ...
  componentDidMount()
  {
    axios.get(`/api/admin/list_hospitals?token=${this.state.token}`)
    .then(response => {
  ...
  handlePageChangeHospitals(pageNumber) {
    ...
    axios.get(`/api/admin/list_hospitals?token=${this.state.token}&page=`+pageNumber)
    .then(response => {
      return response;
    })
    .then(json => {
  ...
  // //////////////////// VIEW HOSPITAL ///////////////////////////////////////////////////////////
  toggleHospitalView = (hospital_id, name) => {
  ...
  viewHospital(hospital_id, name) {
    axios.get(`/api/admin/hospital/get/`+hospital_id+`?token=${this.state.token}`)
    .then(response => {
    ...
    .then(json => {
      if (json.data.success) {
        ...
        }, this.toggleHospitalView(hospital_id, name));
      } else {
        this.setState({
  ...
  // /////////////////////////////// VIEW DOCTORS ///////////////////////////////////////////
  toggleViewDoctors = (hospital_id, name) => {
    // togglePatientRec(id, name) {
    
    // console.log(hospital_id);
    this.setState({
      viewDoctors: !this.state.viewDoctors,
      hospital_id: hospital_id,
      hospital_name: name,
    });
  ...
  viewDoctors(hospital_id, name) {
    this.setState({
      hospital_id: hospital_id,
      hospital_name: name,
    });
    axios.get(`/api/admin/hospital/doc/`+hospital_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("ROI Cartoon");
      console.log(response);
      return response;
    ...
    .then(json => {
      if (json.data.success) {
        ...
        }, this.toggleViewDoctors(hospital_id, name));
      } else {
  ...
  handlePageChangeDoctors(pageNumber) {
    console.log(`active page is ${pageNumber}`);
    // this.setState({activePage: pageNumber});
    axios.get(`/api/admin/hospital/doc/`+this.state.hospital_id+`?token=${this.state.token}&page=`+pageNumber)
    .then(response => {
      return response;
    })
    .then(json => {
  ...
  changeHospitalStatus(hospital_id){
    this.setState({ 
      hospital_id: hospital_id
    });
    axios.get(`/api/admin/hospital/change_status/`+hospital_id+`?token=${this.state.token}`)
    .then(response => {
      console.log("ROI Cartoon");
      console.log(response);
      return response;
  ...
  render() {
    ...
    if(this.state.status_hospital == "0"){ this.state.status_hospital = "Not activated" }
    if(this.state.status_hospital == "1"){ this.state.status_hospital = "Activated" }
      ...
      <div className="animated fadeIn"> 
                  ...
                  <tbody>
                    {
                      ...
                      this.state.hospitals_list.map(hospital=>{
                        if(hospital.status == 1){
                          this.state.doc_status = <Button size="sm" onClick={() => this.changeHospitalStatus(hospital.id)} active color="danger" title="deactivate account" aria-pressed="true"><i className="fa fa-close"></i></Button>;
                        }else{
                          this.state.doc_status = <Button size="sm" onClick={() => this.changeHospitalStatus(hospital.id)} active color="success" title="activate account" aria-pressed="true"><i className="fa fa-check"></i></Button>;
                        }
                        ...
                        return(
                          <tr key={hospital.id}>
                            ...
                            <td>{hospital.country_of_residence}</td>
                            {/* <td>{this.state.status}</td> */}
                            <td>
                              <Button size="sm" onClick={() => this.viewHospital(hospital.id, name)} className="btn-facebook btn-brand icon mr-1 mb-1" title="View hospital"><i className="fa fa-user"></i></Button>
                              <Button size="sm" onClick={() => this.viewDoctors(hospital.id, name)} className="btn-vine btn-brand icon mr-1 mb-1" title="View doctors"><i className="fa fa-user-md"></i></Button>
                            </td>
        ...
        <Modal isOpen={this.state.hospitalView} className={'modal-primary ' + this.props.className} style={{maxWidth: "1000px"}}>
          ...
          <ModalBody>
            ...
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i>{this.state.patient_name} Hospital's Data
              </CardHeader>
              <CardBody>
                <Row>
                  <Col xs="12" sm="12">
        ...
        {/* //////////// DOCTORS  //////////////////////////////////////////////////////// */}
        <Modal isOpen={this.state.viewDoctors} className={'modal-primary ' + this.props.className} style={{maxWidth: "1200px"}}>
          ...
          <ModalBody>
            ...
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i>{this.state.patient_name} Doctors
              </CardHeader>
              <CardBody>
                <Row>  
                  {/* ///////// Doctors TABLE ///////////// */}
