...
/***/ "./resources/coreui/src/views/PharmAccount/PharmAccount.js":
/*!*****************************************************************!*\
  !*** ./resources/coreui/src/views/PharmAccount/PharmAccount.js ***!
  \*****************************************************************/
...
var PharmAccount = /*#__PURE__*/function (_Component) {
  _inherits(PharmAccount, _Component);

  var _super = _createSuper(PharmAccount);

  function PharmAccount(props) {
    var _this$state;

    var _this;

    _classCallCheck(this, PharmAccount);
    ...
    _this.state = (_this$state = {
      ...
      currentPage2: '',
      status: '1'
    }, _defineProperty(_this$state, "status", ""), _defineProperty(_this$state, "created_at", localStorage["appState"] ? JSON.parse(localStorage["appState"]).user.created_at : ""), _defineProperty(_this$state, "user_type", localStorage["appState"] ? JSON.parse(localStorage["appState"]).user.user_type : ""), _this$state);
    _this.handlePageChange = _this.handlePageChange.bind(_assertThisInitialized(_this));
    return _this;
  } // fetch data from db


  _createClass(PharmAccount, [{
    key: "componentDidMount",
    value: function componentDidMount() {
      var _this2 = this;

      axios__WEBPACK_IMPORTED_MODULE_3___default.a.get("/api/pharm/order/account/" + this.state.id + "?token=".concat(this.state.token)).then(function (response) {
        return response;
      }).then(function (json) {
    ...
    value: function handlePageChange(pageNumber) {
      ...
      axios__WEBPACK_IMPORTED_MODULE_3___default.a.get("/api/pharm/order/account/" + this.state.id + "?token=".concat(this.state.token, "&page=") + pageNumber).then(function (response) {
        return response;
      }).then(function (json) {
        if (json.data.success) {
          _this3.setState({
    ...
    value: function render() {
      var _this4 = this; // const { product_image} = this.state
      ...
      this.state.applications_list.map(function (application) {
        if (application.status == 1) {
          _this4.state.status = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement(reactstrap__WEBPACK_IMPORTED_MODULE_5__["Badge"], {
            color: "danger"
...
/* harmony default export */ __webpack_exports__["default"] = (PharmAccount);
